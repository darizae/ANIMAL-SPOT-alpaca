...
def main(args: argparse.Namespace) -> None:
    ...
    # If TRAINING_ROOT is .../TRAINING/runs, models live under models/*/models/ANIMAL-SPOT.pk
    model_pk_glob = "models/*/models/ANIMAL-SPOT.pk"
    for model_pk in training_root.glob(model_pk_glob):
        model_dir = model_pk.parent.parent  # .../models/vX_*   (the variant dir under runs/models)
        model_name = model_dir.name

        pred_cfgs: list[Path] = []
        # (eval cfgs are written too, though the sbatch here only does prediction; eval is a separate step)
        for var in variants:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--training-root", help="Path to TRAINING runs root (default: $TRAINING_ROOT)")
    ap.add_argument("--benchmark-root", help="Path to BENCHMARK root (default: $BENCHMARK_ROOT)")
    ap.add_argument("--corpus-root", required=True, help="Benchmark corpus folder (e.g. benchmark_corpus_v1)")
